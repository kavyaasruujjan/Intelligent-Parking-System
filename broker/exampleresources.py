...
class AdvancedResource(Resource):
	...
	print self.park_array
	test = request.uri_path.split('/')
	test2 = test[-1]
	print test2
	response.payload = self.park_array[int(test2)]
	print self.park_array[int(test2)]
	...
	print len(test)
	if len(test) == 2: 
		self.park_array.append(request.payload)
		print self.park_array
	if len(test) == 1:
		response.payload = str(self.id_count)
		self.id_count = self.id_count + 1
	self.payload = request.payload
        return self, response
	...
	test = request.uri_path.split('/')
	test2 = test[-1]
	self.park_array[int(test2)] = request.payload 
